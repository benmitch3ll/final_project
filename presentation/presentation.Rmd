...
```{r include=FALSE}
#model 1
countries_fit_1 <- linear_reg() %>%
  set_engine("lm") %>%
  fit(happiness_score ~ economy + freedom + family + trust + health, data = happiness_countries)

```

```{r include=FALSE}
broom::augment(countries_fit_1$fit)
#predict(countries_fit_1, happiness_countries)

#countries_fit_1_aug <- broom::augment(countries_fit_1$fit)


#ggplot(countries_fit_1_aug, mapping = aes(x = .fitted, y = .resid))+
#  geom_point(alpha = 0.5)+
#  geom_hline(yintercept = 0, color = "gray", lty = #"dashed")+
#  labs(x = "Predicted Happiness Score", y = "Residuals")
```
